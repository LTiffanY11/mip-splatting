...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Progressive 3D smoothing filter schedule
        # Early phase (0-15K): weak filter (30%) to allow free exploration
        # Late phase (15K-30K): linearly increase to full strength (100%)
        if iteration < 15000:
            filter_scale = 0.3
        else:
            filter_scale = 0.3 + 0.7 * (iteration - 15000) / 15000.0
        gaussians.set_filter_3D_scale(filter_scale)
